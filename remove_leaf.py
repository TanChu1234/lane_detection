...
def annotate_image(image_in):
    ...
    vertices = np.array([[\
    (0, imshape[0]),\
    (0, imshape[0]/3 + imshape[0] * 0.1),\
    (imshape[1]/2 - imshape[1] * 0.1, imshape[0]/2 - imshape[0] * 0.2),\
    (imshape[1]/2 + imshape[1] * 0.1, imshape[0]/2 - imshape[0] * 0.2),\
    (imshape[1], imshape[0]/3 + imshape[0] * 0.1),\
    ((imshape[1], imshape[0]))]]\
    , dtype=np.int32)
...
for i in range(0,2):
    # img = cv2.resize(cv2.imread(r"D:\Lane Detection\lane_detection-master\lane_detection-master\img\pic0.jpg"), (640,480))
    img = cv2.resize(cv2.imread("D:\Lane Detection\lane_detection-master\lane_detection-master\img\pic" + str(i)+".jpg"), (640,480))
